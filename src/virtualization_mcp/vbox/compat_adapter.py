...
class VBoxManager:
    ...
    def create_vm(self, name: str = None, ostype: str = None, memory: int = None, cpus: int = None, **kwargs) -> Dict[str, Any]:
        """
        Create a new VM.
        ...
        try:
            ...
            if disk_size_mb > 0:
                ...
                self.vbox._run_command(f'createhd --filename {disk_path} --size {disk_size_mb // 1024} --variant Standard')
                
                # Attach the disk to the VM
                self.vbox._run_command(f'storageattach "{name}" --storagectl "SATA Controller" --port 0 --device 0 --type hdd --medium {disk_path}')
    ...
    def execute_in_vm(self, vm_name: str, command: List[str], **kwargs) -> str:
        ...
        try:
            ...
            cmd_parts = [
                'guestcontrol', vm_name, 'execute',
                '--username', username,
                '--password', password,
                '--wait-exe', str(timeout_ms // 1000),  # Convert to seconds
                '--timeout', str(timeout_ms // 1000),
                '--',
            ...
            # Execute the command
            return self.vbox._run_command(' '.join(f'"{part}"' if ' ' in str(part) else str(part) for part in cmd_parts))
    ...
    def restore_snapshot(self, vm_name: str, snapshot_name: Optional[str] = None, snapshot_uuid: Optional[str] = None, **kwargs) -> bool:
        """
    ...
    def delete_snapshot(self, vm_name: str, snapshot_name: Optional[str] = None, snapshot_uuid: Optional[str] = None) -> bool:
        """
        Delete a snapshot.
