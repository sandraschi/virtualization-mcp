...
async def list_snapshots(vm_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "vm_name": vm_name, "snapshots": snapshots}

    except subprocess.CalledProcessError as e:
        logger.error(f"Error listing snapshots: {e}")
        return {"status": "error", "message": f"Failed to list snapshots: {e.stderr}"}


async def get_snapshot_info(vm_name: str, snapshot_name: str) -> dict[str, Any]:
    """
    ...
    try:
        ...
        return {"status": "success", "snapshot_info": details}

    except subprocess.CalledProcessError as e:
        logger.error(f"Error getting snapshot info: {e}")
        return {"status": "error", "message": f"Failed to get snapshot info: {e.stderr}"}
...
async def delete_snapshot(vm_name: str, snapshot_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "message": f"Snapshot '{snapshot_name}' deleted successfully"}

    except subprocess.CalledProcessError as e:
        logger.error(f"Error deleting snapshot: {e}")
        return {"status": "error", "message": f"Failed to delete snapshot: {e.stderr}"}


async def restore_current_snapshot(vm_name: str, start_vm: bool = False) -> dict[str, Any]:
    """
