        ...
        try:
            ...
            try:
                ...
                if progress.resultCode != 0:
                    return {
                        "status": "error",
                        "message": f"Backup failed: {progress.errorInfo.text}",
                    }
...
class BackupManager:
    ...
    def list_backups(self, vm_name: str | None = None) -> list[dict]:
        """
        List all backups, optionally filtered by VM name.
    ...
    def cleanup_old_backups(self, days_old: int = 30, max_backups: int | None = None) -> dict:
        """
