...
class SandboxTester:
    ...
    async def _load_existing_sandboxes(self):
        ...
        for entry in self.base_dir.iterdir():
            if entry.is_dir() and (entry / "sandbox.json").exists():
                try:
                    with open(entry / "sandbox.json", 'r') as f:
                        data = json.load(f)
    ...
    async def create_sandbox(
        self,
        name: str,
        sandbox_type: Union[SandboxType, str] = SandboxType.VENV,
        requirements: Optional[List[str]] = None,
        environment: Optional[Dict[str, str]] = None,
        resource_limits: Optional[Union[Dict, ResourceLimits]] = None,
        network_config: Optional[Union[Dict, NetworkConfig]] = None,
        persistent_storage: bool = False,
        timeout: int = 300,
        **kwargs
    ) -> SandboxInfo:
    ...
    async def cleanup_sandbox(self, name: str, force: bool = False) -> bool:
        ...
        if sandbox and sandbox.state == SandboxState.RUNNING and not force:
            raise RuntimeError(
                f"Cannot clean up running sandbox '{name}'. "
    ...
    async def _check_resource_overages(self, sandbox: SandboxInfo) -> bool:
        ...
        sandbox = self._active_sandboxes.pop(name)
        sandbox_dir = Path(sandbox["directory"])
        
        try:
            # Stop any running containers/processes
            if sandbox["type"] == SandboxType.DOCKER:
                await self._stop_docker_container(name)
