...
class MetricsManager:
    ...
    def record_error(self, error_type: str, message: str, details: dict | None = None) -> None:
    ...
    def get_metrics(self, time_range: int = 300) -> dict[str, Any]:
        ...
        with self.lock:
            ...
            system_metrics = [m for m in self.system_metrics if m.timestamp >= cutoff]
            ...
            # Filter API requests
            api_requests = [r for r in self.api_requests if r["timestamp"] >= cutoff]
...
def start_metrics_server(
    ...
    prometheus_thread = threading.Thread(target=prometheus_metrics.start_http_server, daemon=True)
...
def record_error(error_type: str, message: str, details: dict | None = None) -> None:
    """
