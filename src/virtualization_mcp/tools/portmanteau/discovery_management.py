...
def register_discovery_management_tool(mcp: FastMCP) -> None:
    ...
    async def discovery_management(
        ...
        """
        Get help and tool information for virtualization-mcp with various actions.

        This portmanteau consolidates application-specific help and introspection tools.
        Provides information about available tools, their operations, and usage.
        
        Note: This is separate from MCP protocol's native tools/list method.
        MCP clients get tool schemas automatically - this tool provides app-specific
        help content and detailed introspection for users.
        ...
        Args:
            action: The operation to perform. Available actions:
                - list_tools: List all virtualization-mcp tools with their operations
                    ...
                    Optional filters: category (vm, network, snapshot, storage, system)
                    Optional search: filter by name or description
                    
                - tool_info: Get detailed information about a specific tool
                    Requires: tool_name parameter
                    Returns: tool type, operations list, description
                    Works for any portmanteau tool (vm_management, network_management, etc.)
                    
                - tool_schema: Get JSON schema information for a tool's parameters
                    Requires: tool_name parameter
                    Returns: schema information and notes about Literal types
                    Note: Actual schemas auto-generated by FastMCP from type hints
                    
                - help: Get general help and server information
                    No parameters required
                    Returns: server version, tool modes, quick start info, documentation links

            tool_name: Name of the tool to get info/schema for (required for tool_info, tool_schema)
            ...
            category: Filter tools by category (optional, for list_tools action)
                Valid values: vm, network, snapshot, storage, system, discovery, hyperv
                
            search: Search term to filter tools by name or description (optional, for list_tools)

        Returns:
            Dictionary containing:
            - success: bool - Operation success status
            - For list_tools: tools (list), count (int), filters (dict)
            - For tool_info: tool_name (str), info (dict with operations, description)
            - For tool_schema: tool_name (str), message (str), note (str)
            - For help: help (dict with server info, modes, quick start)
            - error: str - Error message if operation failed
            - available_actions: dict - Available actions if invalid action provided
        ...
        Examples:
            ...
            # Get detailed info about vm_management tool
            result = await discovery_management(
                action="tool_info",
            ...
            # Get schema info for a tool
            result = await discovery_management(
                action="tool_schema",
                tool_name="snapshot_management"
            )

            # Get general help and server information
