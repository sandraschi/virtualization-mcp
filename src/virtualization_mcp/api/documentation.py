...
class APIDocumentation:
    ...
    def _generate_endpoint_doc(self, tool) -> dict:
        ...
        if hasattr(tool, "body_schema"):
            request_body = {
                "required": True,
                "content": {"application/json": {"schema": tool.body_schema}},
            }
    ...
    def _get_schema_for_type(self, type_hint) -> dict:
        ...
        if type_hint in (str, "str"):
            ...
            return {"type": "array", "items": self._get_schema_for_type(type_hint.__args__[0])}
    ...
    def _get_success_response_schema(self, tool) -> dict:
        ...
        if return_type == "VM":
            ...
            return {"type": "array", "items": {"$ref": "#/components/schemas/VM"}}
        elif return_type == "Snapshot":
            return {"$ref": "#/components/schemas/Snapshot"}
        elif return_type == "List[Snapshot]":
            return {"type": "array", "items": {"$ref": "#/components/schemas/Snapshot"}}
        else:
            return {
                "type": "object",
                "properties": {
...
def register_documentation_routes(mcp: FastMCP) -> None:
    ...
    async def get_tool_categories() -> dict:
        ...
        Example:
            ...
            ```
        """
        return {"status": "success", "data": TOOL_CATEGORIES}
    ...
    async def get_api_endpoints(
        ...
        def matches_criteria(
            ...
            if category:
                ...
                if not (
                    category_lower in endpoint_lower
                    or category_lower in tool_name.lower()
                    or (tool_description and category_lower in tool_description.lower())
                ):
                    return False
