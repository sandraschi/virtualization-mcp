...
from ..vbox.compat_adapter import get_vbox_manager, VBoxManagerError
...
class VMService:
    ...
    def get_vm_state(self, vm_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "troubleshooting": [
                    f"Use list_vms() to see all available VMs",
                    f"Check VirtualBox logs if the state seems incorrect"
                ]
            }
        ...
        try:
            ...
            if current_state in ['poweroff', 'aborted']:
                return {
                    ...
                    "troubleshooting": [
                        "No action was taken as the VM is not running"
                    ]
                }
            ...
            if current_state not in ['running', 'paused']:
                raise VBoxManagerError(
                    f"Cannot stop VM in state '{current_state}'. "
            ...
            response = {
                ...
                "troubleshooting": [
                    ...
                    f"Check VM state with: list_vms()"
                ]
            }
            ...
            if force:
                response["warning"] = (
                    "Forced power-off may cause data loss. "
    ...
    def delete_vm(self, name: str, delete_disk: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if current_state not in ['poweroff', 'aborted', 'saved']:
                raise VBoxManagerError(
                    f"Cannot delete VM in state '{current_state}'. "
                    "VM must be powered off or saved."
                )
            ...
            if not delete_disk and disk_paths:
                ...
                response["troubleshooting"].append(
                    f"Retained disk files: {', '.join(disk_paths)}"
                )
        ...
        try:
            ...
            if start_vm:
                ...
                if start_result['status'] != 'success':
                    raise VBoxManagerError(f"Failed to start VM after restore: {start_result.get('error', 'Unknown error')}")
    ...
    def attach_disk(self, vm_name: str, disk_path: str, port: int = 0, 
                   device: int = 0, type: str = "hdd") -> Dict[str, Any]:
        """
    ...
    def detach_disk(self, vm_name: str, port: int, device: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Detach the disk
            result = self.vm_operations.detach_disk(
                vm_name=vm_name,
                port=port,
                device=device
            )
    ...
    def configure_shared_folder(self, vm_name: str, folder_name: str, 
                              host_path: str, auto_mount: bool = True, 
                              mount_point: str = "") -> Dict[str, Any]:
        """
    ...
    def export_vm(self, vm_name: str, output_path: str, 
                 format: str = "ovf-1.0") -> Dict[str, Any]:
        """
    ...
    def clone_vm(self, source_vm: str, new_vm_name: str, 
                clone_type: str = "full") -> Dict[str, Any]:
        """
    ...
    def mount_iso(self, vm_name: str, iso_path: str, controller_name: str = 'IDE Controller', 
                 port: int = 1, device: int = 0) -> Dict[str, Any]:
        """
    ...
    def unmount_iso(self, vm_name: str, controller_name: str = 'IDE Controller',
                   port: int = 1, device: int = 0) -> Dict[str, Any]:
        """
    ...
    def configure_network_adapter(self, vm_name: str, adapter_id: int, enabled: bool = True,
                                network_type: str = 'nat', mac_address: str = None,
                                cable_connected: bool = True) -> Dict[str, Any]:
        """
    ...
    def create_disk(self, disk_path: str, size_gb: int, format_type: str = 'vdi',
                   variant: str = 'Standard', resizeable: bool = True) -> Dict[str, Any]:
        """
    ...
    def attach_disk(self, vm_name: str, disk_path: str, controller_name: str = 'SATA Controller',
                   port: int = 0, device: int = 0, disk_type: str = 'hdd') -> Dict[str, Any]:
        """
    ...
    def take_screenshot(self, vm_name: str, output_file: str = None,
