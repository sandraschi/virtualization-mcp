...
class VMLifecycleMixin:
        ...
        try:
            ...
            if current_state in ["poweroff", "aborted", "saved"]:
                return {
                    ...
                    "troubleshooting": ["No action needed - the VM is already in a stopped state"],
                }
    ...
    def delete_vm(self, name: str, delete_disks: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            if current_state == "running":
                raise VBoxManagerError(f"Cannot delete running VM '{name}'. Stop the VM first.")
            ...
            if not delete_disks and disk_files:
                ...
                response["troubleshooting"].append(f"{len(disk_files)} disk files were not deleted")
