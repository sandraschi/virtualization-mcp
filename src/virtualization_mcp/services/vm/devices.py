...
class VMDeviceMixin:
    ...
    @device_operation
    def attach_iso(self, vm_name: str, iso_path: str, port: int = 1, 
                  device: int = 0, controller: str = "IDE Controller", 
    ...
    def _attach_iso_virtualbox(self, vm_name: str, iso_path: str, port: int, 
                             device: int, controller: str, temporary: bool) -> Dict[str, Any]:
            ...
            if not storage_ctl:
                ...
                if "IDE" in controller:
                    storage_ctl = session.machine.add_storage_controller(
                        controller, 
                        self.vbox_manager.constants.StorageBus_IDE
                    )
                else:
    ...
    def _attach_iso_hyperv(self, vm_name: str, iso_path: str, port: int, 
                          device: int, temporary: bool) -> Dict[str, Any]:
    ...
    @device_operation
    def detach_iso(self, vm_name: str, port: int = 1, device: int = 0, 
                  controller: str = "IDE Controller") -> Dict[str, Any]:
        """
    ...
    def _detach_iso_virtualbox(self, vm_name: str, port: int, device: int, 
    ...
    @device_operation
    def attach_usb(self, vm_name: str, usb_filter: str, port: int = 0, 
                  capture: bool = True, echi: bool = False) -> Dict[str, Any]:
        """
    ...
    def _attach_usb_virtualbox(self, vm_name: str, usb_filter: str, port: int, 
                             capture: bool, echi: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            if not usb_controller:
                usb_controller = session.machine.add_usb_controller(
                    'USB Controller', 
                    self.vbox_manager.constants.USBControllerType_OHCI
                )
            ...
            if echi and usb_controller.type == self.vbox_manager.constants.USBControllerType_OHCI:
                ...
                usb_controller = session.machine.add_usb_controller(
                    'USB Controller', 
                    self.vbox_manager.constants.USBControllerType_EHCI
                )
    ...
    def _attach_usb_hyperv(self, vm_name: str, usb_filter: str, port: int, 
        ...
        try:
            ...
            ps_script = [
                ...
                f'$vm | Add-VMScsiController -ErrorAction Stop;',
                f'$vm | Add-VMHardDiskDrive -ControllerType SCSI -ErrorAction Stop;',
    ...
    def _detach_usb_virtualbox(self, vm_name: str, usb_filter: str = None, port: int = None) -> Dict[str, Any]:
    ...
    def _detach_usb_hyperv(self, vm_name: str, usb_filter: str = None, port: int = None) -> Dict[str, Any]:
    ...
    def _list_devices_virtualbox(self, vm_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                    ...
                    for port in range(controller.port_count):
                        for device in range(controller.max_devices_per_port):
                            try:
                                ...
                                logger.debug(f"No device at {controller.name}:{port}:{device}: {str(e)}")
                ...
                if session and hasattr(session, 'unlock_machine') and \
                   session.state == self.vbox_manager.constants.SessionState_Locked:
                    try:
                        session.unlock_machine()
    ...
    def _update_network_adapter_virtualbox(self, vm_name: str, adapter_number: int, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(k in kwargs for k in ['nat_network', 'bridged_interface', 'internal_network', 'host_only_interface']):
    ...
    def _update_network_adapter_hyperv(self, vm_name: str, adapter_number: int, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'mac_address' in kwargs:
                updates.append(f'Set-VMNetworkAdapter -VMNetworkAdapter $adapter -StaticMacAddress "{kwargs["mac_address"]}"')
                
            if 'switch_name' in kwargs:
                updates.append(f'Connect-VMNetworkAdapter -VMNetworkAdapter $adapter -SwitchName "{kwargs["switch_name"]}"')
    ...
    @device_operation
    def add_disk(self, vm_name: str, disk_path: str, disk_type: str = 'normal', 
                size_mb: int = 10240, controller: str = 'SATA Controller', 
                port: int = 0, device: int = 0) -> Dict[str, Any]:
        """
