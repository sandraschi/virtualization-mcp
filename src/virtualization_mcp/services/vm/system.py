...
class SystemSettingsManager(BasePlugin):
    ...
    def setup_routes(self):
        ...
        self.router.add_api_route(
            "/{vm_name}", self.get_system_settings, methods=["GET"], response_model=SystemSettings
        )
        self.router.add_api_route(
            "/{vm_name}",
        ...
        self.router.add_api_route(
            "/{vm_name}/boot-order", self.get_boot_order, methods=["GET"], response_model=list[str]
        )
        self.router.add_api_route(
            "/{vm_name}/boot-order", self.set_boot_order, methods=["PUT"], response_model=list[str]
        )
    ...
    async def set_boot_order(self, vm_name: str, boot_order: list[str]) -> list[str]:
