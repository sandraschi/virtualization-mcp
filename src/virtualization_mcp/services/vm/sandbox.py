...
class VMSandboxManager:
    ...
    def _clone_vm_virtualbox(self, source_vm: str, clone_name: str, target_path: str) -> Dict[str, Any]:
        """
    ...
    def _isolate_network_virtualbox(self, vm_name: str) -> None:
        ...
        try:
            ...
            subprocess.run(
                ['VBoxManage', 'modifyvm', vm_name, '--nic1', 'hostonly', '--hostonlyadapter1', internal_net_name],
                check=True,
                capture_output=True
            )
    ...
    def _apply_limits_hyperv(self, vm_name: str, limits: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if 'memory_mb' in limits:
                ...
                script.append(f"Set-VMMemory -VMName '{vm_name}' -DynamicMemoryEnabled $false -StartupBytes {memory_bytes}")
            ...
            if 'cpu_cap' in limits:
                ...
                script.append(f"Set-VMProcessor -VMName '{vm_name}' -Maximum ${{env:NUMBER_OF_PROCESSORS}} -Reserve 10 -MaximumCountPerNumaNode ${{env:NUMBER_OF_PROCESSORS}} -MaximumCountPerNumaSocket ${{env:NUMBER_OF_PROCESSORS}} -RelativeWeight {weight}")
            
            # Execute the script if we have any commands
            if len(script) > 1:
                subprocess.run(
                    ['powershell', '-Command', ';'.join(script)],
                    check=True,
                    capture_output=True
                )
    ...
    def _delete_vm_hyperv(self, vm_name: str) -> None:
        ...
        try:
            # Stop the VM if it's running
            subprocess.run(
                ['powershell', '-Command', f"Stop-VM -Name '{vm_name}' -Force -ErrorAction SilentlyContinue"],
                check=False
            )
