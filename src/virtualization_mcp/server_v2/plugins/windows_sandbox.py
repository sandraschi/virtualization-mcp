...
from ..utils.windows_sandbox_helper import WindowsSandboxError, WindowsSandboxHelper
...
class WindowsSandboxPlugin(BasePlugin):
    ...
    def setup_routes(self) -> None:
        ...
        async def create_sandbox(
            ...
            # Clean up when done
            task.add_done_callback(lambda t, n=name: self._cleanup_sandbox(n))
        ...
        async def execute_in_sandbox(
            ...
            try:
                ...
                return {"status": "success", "sandbox": name, "command": command, "result": result}
            except Exception as e:
                raise HTTPException(
    ...
    async def _execute_in_sandbox(
        ...
        try:
            ...
            result = await self.sandbox_helper.execute_command(
                sandbox_name=name, command=command, wait=wait, timeout=timeout
            )
    ...
    async def _upload_to_sandbox(self, name: str, file: UploadFile, destination: str) -> str:
        ...
        try:
            ...
            result = await self.sandbox_helper.upload_file(
                sandbox_name=name, source_path=temp_file, destination_path=destination
            )
    ...
    async def _terminate_sandbox(self, name: str, force: bool = False) -> None:
        ...
        try:
            ...
            result = await self.sandbox_helper.terminate_sandbox(sandbox_name=name, force=force)
