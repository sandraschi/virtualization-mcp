...
export default function VirtualBox() {
  ...
  const [unattendedDevTools, setUnattendedDevTools] = useState<
  ...
  const fetchSnapshots = async (vmName: string) => {
    try {
      const res = await fetch(
        `${API_BASE}/api/v1/vms/${encodeURIComponent(vmName)}/snapshots`,
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const submitTemplate = async () => {
    ...
    try {
      let config;
      try {
        config = JSON.parse(editTemplateConfig);
      } catch {
  ...
  const deleteTemplate = async (name: string) => {
    try {
      await fetch(`${API_BASE}/api/v1/templates/${encodeURIComponent(name)}`, {
        method: "DELETE",
      });
      fetchTemplates();
    } catch {
  ...
  const fetchNetworkConfig = async (vmName: string) => {
    try {
      const res = await fetch(
        `${API_BASE}/api/v1/vms/${encodeURIComponent(vmName)}/network`,
      );
      if (res.ok) {
        const d = await res.json();
  ...
  return (
    <div className="space-y-6 pb-8">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          ...
          >
            ...
            <p className="text-sm text-muted-foreground">
              ...
              <code className="text-foreground/80">assets/vbox/</code> and
              appear in the ISO dropdown automatically.
            </p>
            {/* Horizontal tabs */}
                              ...
                              <div className="text-right min-w-[120px]">
                                ...
                                {activeTask.status === "failed" && (
                                  <div className="text-right">
                                    ...
                                    {activeTask.error && (
                                      <p
                                        className="text-xs text-red-400/70 max-w-[180px] truncate"
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div
                    className={`relative p-3 rounded-xl ${vm.state === "running" ? "bg-green-500/15 text-green-400" : "bg-muted/50 text-muted-foreground"}`}
                  >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-foreground/70 mt-1 font-medium">
                      {vm.os_type || "Unknown OS"} &bull; {vm.memory_mb || "?"}
                      MB RAM &bull; {vm.cpus || "?"} vCPUs
                    </p>
                  </div>
                </div>
                {/* Primary actions */}
                  ...
                  <div className="flex items-center gap-1 mr-1 pr-2 border-r border-border/50">
                    ...
                    {[1, 2, 3].map((d) => (
                      <button
                        ...
                        onClick={async () => {
                          await fetch(
                            `${API_BASE}/api/v1/vms/${encodeURIComponent(vm.name)}/move-desktop?desktop=${d}`,
                            { method: "POST" },
                          );
                        }}
                  ...
                  {vm.state === "running" ? (
                    <>
                      <button
                        ...
                        disabled={actionId === `${vm.name}pause`}
                      ...
                      <button
                        onClick={() =>
                          handleAction(vm.name, "stop", vm.provider)
                        }
                        disabled={actionId === `${vm.name}stop`}
                    ...
                    <button
                      ...
                      disabled={actionId === `${vm.name}start`}
              ...
              <div className="flex items-center gap-2 mt-4 pt-3 border-t border-border/40">
                <button
                  ...
                  disabled={actionId === `${vm.name}snapshot`}
                ...
                {vm.provider === "virtualbox" && (
                  <button
                    onClick={() => {
                      setShowNetworkConfig(vm.name);
                      fetchNetworkConfig(vm.name);
                    }}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg bg-white/5 text-muted-foreground hover:bg-white/15 hover:text-white transition-colors font-medium"
                  >
                    Network
                ...
                {vm.state === "running" && (
                  <button
                    onClick={async () => {
                      await fetch(
                        `${API_BASE}/api/v1/vms/${encodeURIComponent(vm.name)}/vrde`,
                        {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                ...
                <button
                  onClick={() => {
                    setShowUnattended(vm.name);
                    setUnattendedDevTools({
                      python: true,
                      git: true,
                      node: true,
                      vscode: true,
                ...
                {vm.state !== "running" && (
                  <button
                    onClick={async () => {
                      ...
                      try {
                        await fetch(
                          `${API_BASE}/api/v1/vms/${encodeURIComponent(vm.name)}`,
                          { method: "DELETE" },
                        );
                        setTimeout(fetchVMs, 1000);
                      } catch {}
                  ...
                  {snapshots[vm.name]?.length > 0 ? (
                    <div className="space-y-2">
                      {snapshots[vm.name].map((snap: any, i: number) => (
                        <div
                          key={i}
                          className="flex items-center justify-between py-2 px-3 rounded-lg bg-black/20 border border-border/40"
                        >
                          <div className="min-w-0">
                          ...
                          <div className="flex gap-2 ml-3">
                            <button
                              onClick={async () => {
                                await fetch(
                                  `${API_BASE}/api/v1/vms/${encodeURIComponent(vm.name)}/restore`,
                                  {
                                    method: "POST",
                                    headers: {
                              ...
                              className="px-3 py-1 text-xs rounded-md bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500/30 transition-colors font-medium"
                            >
                              Restore
                            </button>
                            <button
                              onClick={async () => {
                                await fetch(
                                  `${API_BASE}/api/v1/vms/${encodeURIComponent(vm.name)}/delete-snapshot`,
                                  {
                                    method: "POST",
                                    headers: {
                                      "Content-Type": "application/json",
                              ...
                              className="px-3 py-1 text-xs rounded-md bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-colors font-medium"
                            >
                              Delete
                            </button>
                          </div>
            ...
            <div>
              ...
              {vboxAssets.length === 0 && (
                <p className="text-xs text-muted-foreground mt-1">
                  Use the <strong>Download ISOs</strong> button above to
                  download ISOs, or put .iso files in repo assets/vbox folder.
                </p>
              )}
            </div>
            <div>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              {vboxAssets.length === 0 && (
                <p className="text-xs text-muted-foreground mt-1">
                  Use the <strong>Download ISOs</strong> button to download
      ...
      {showTemplateManager && (
        ...
        >
          <div
            className="bg-card border border-border rounded-xl shadow-xl max-w-lg w-full max-h-[80vh] overflow-y-auto p-6 space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between">
            ...
            <div className="space-y-2">
              ...
              />
              <textarea
                value={editTemplateConfig}
                onChange={(e) => setEditTemplateConfig(e.target.value)}
                placeholder='{"os_type":"Ubuntu_64","memory_mb":4096,"disk_gb":25,"cpus":2,"network":"NAT"}'
                rows={4}
                className="w-full bg-background/50 border border-input rounded px-3 py-2 text-xs font-mono"
              />
              <button
                onClick={submitTemplate}
                disabled={creatingTemplate || !editTemplateName.trim()}
              ...
              {templates.map((t: any) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t.config?.os_type || t.os_type} ·{" "}
                      {t.config?.memory_mb || t.memory_mb || "?"}MB
                    </p>
                  </div>
                  {!t.builtin && (
                    <button
      ...
      {showNetworkConfig && (
        ...
        >
          <div
            className="bg-card border border-border rounded-xl shadow-xl max-w-lg w-full p-6 space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-lg">
                Network: {showNetworkConfig}
              </h3>
              <button
                onClick={() => setShowNetworkConfig(null)}
                className="p-1 rounded hover:bg-white/10 text-muted-foreground"
                ...
                {vmNetworks[showNetworkConfig]?.adapters?.map((a: any) => (
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={async (e) => {
                        await fetch(
                          `${API_BASE}/api/v1/vms/${encodeURIComponent(showNetworkConfig)}/network`,
                          {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({
            ...
            <div className="border-t border-border pt-3">
              ...
              {vmNetworks[showNetworkConfig]?.port_forwarding?.length > 0 ? (
                <div className="space-y-1 mb-2">
                  {vmNetworks[showNetworkConfig]?.port_forwarding?.map(
                    (r: any, i: number) => (
                      <div
                        key={i}
                      ...
                      >
                        <span>
                          {r.name}: {r.protocol}://host:{r.host_port} → guest:
                          {r.guest_port}
                        </span>
                        <button
                          onClick={async () => {
                            await fetch(
                              `${API_BASE}/api/v1/vms/${encodeURIComponent(showNetworkConfig)}/network/port-forwarding/${encodeURIComponent(r.name)}`,
                              { method: "DELETE" },
                            );
                            fetchNetworkConfig(showNetworkConfig!);
                          }}
      ...
      {showUnattended && (
        ...
        >
          <div
            className="bg-card border border-border rounded-xl shadow-xl max-w-lg w-full max-h-[85vh] overflow-y-auto p-6 space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="font-semibold text-lg">
              Autoinstall: {showUnattended}
            </h3>
            <p className="text-sm text-muted-foreground">
            ...
            <div>
              ...
              <input
                value={unattendedUsername}
                onChange={(e) => setUnattendedUsername(e.target.value)}
                className="w-full bg-background/50 border border-input rounded px-3 py-2"
              />
            </div>
            <div>
            ...
            <div className="border-t border-border pt-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={unattendedDevTools[id] ?? false}
                      onChange={(e) =>
                        setUnattendedDevTools((prev) => ({
                          ...prev,
                          [id]: e.target.checked,
                        }))
                      }
              ...
              <label className="flex items-center gap-2 text-sm mt-2 py-1 px-2 rounded hover:bg-white/5 cursor-pointer">
                <input
                  type="checkbox"
                  checked={unattendedUseOllama}
                  onChange={() => setUnattendedUseOllama(!unattendedUseOllama)}
                  className="rounded border-white/20 bg-white/5"
                />
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                onClick={() => submitUnattended(showUnattended!)}
