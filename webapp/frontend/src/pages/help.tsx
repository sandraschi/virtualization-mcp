...
const fallbackFaqs = [
  {
    ...
    answer:
      "Navigate to the 'Windows Sandbox' page, configure your XML parameters (mapped folders, startup scripts), and click 'Launch Sandbox'. The backend generates a temporary .wsb file and executes it via the system shell.",
    icon: Box,
  },
  {
    question: "Can I manage headless VirtualBox VMs?",
    answer:
      "Yes. The VirtualBox Manager interface detects all registered VMs. Clicking 'Start' will initiate the VM in the mode configured in your VirtualBox global settings (usually GUI or Headless).",
    icon: Server,
  },
  {
    question: "Why are there reserved ports in this app?",
    answer:
      "The app uses reserved local ports so the dashboard, API, and MCP server can find each other reliably without manual wiring.",
    icon: Terminal,
  },
];
...
const containerTech = [
  {
    ...
    notes:
      "Great tooling and ecosystem. Use when you need fast, repeatable app environments.",
  },
  {
    name: "Podman",
    ...
    notes:
      "Strong drop-in option where Docker CLI compatibility matters but daemonless operation is preferred.",
  },
  {
    name: "containerd",
    ...
    notes:
      "Lower-level than Docker. Usually consumed via Kubernetes or managed stacks.",
  },
  {
    name: "Kubernetes",
    summary: "Cluster orchestration for containerized services.",
    bestFor:
      "Production-scale deployments with self-healing and rolling updates.",
    notes:
      "Powerful but complex. Best when you need scaling, scheduling, and policy controls.",
  },
  {
    name: "LXC/LXD",
    ...
    notes:
      "Useful when you need full user-space behavior without running full virtual machines.",
  },
  {
    ...
    notes:
      "Use process or Hyper-V isolation based on compatibility and security requirements.",
  },
];
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground text-lg max-w-xl mx-auto">
          Start here for plain-language setup steps, troubleshooting, and clear
          guidance for VM and container workflows.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <button
            key={item.label}
            className="p-6 rounded-2xl border border-border bg-card/40 backdrop-blur-sm hover:border-primary/30 transition-all flex flex-col items-center gap-4 group"
          >
            <div
              className={`p-4 rounded-xl ${item.bgClass} ${item.iconClass} group-hover:scale-110 transition-transform`}
            >
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h3 className="text-2xl font-bold tracking-tight">
            Container Technologies at a Glance
          </h3>
        </div>
        <p className="text-muted-foreground">
          This section compares common container technologies in simple terms so
          teams can pick the right fit quickly.
        </p>
          ...
          {containerTech.map((tech) => (
            ...
            >
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="text-xs px-2 py-1 rounded-full border border-primary/30 text-primary">
                  Container Tech
                </span>
              </div>
      ...
      <div className="p-10 rounded-3xl bg-gradient-to-br from-primary/20 via-primary/5 to-transparent border border-primary/20 flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="space-y-2 text-center md:text-left">
          ...
          <p className="text-muted-foreground">
            Run diagnostics first, then share results with your platform admin
            if needed.
          </p>
        </div>
