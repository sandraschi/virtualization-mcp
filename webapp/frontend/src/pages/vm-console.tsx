...
export default function VmConsole() {
  ...
  const fetchVrde = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/v1/vms/${encodeURIComponent(name)}/vrde`,
      );
      if (res.ok) {
        const data = await res.json();
        setVrde(data.vrde);
  ...
  useEffect(() => {
    ...
    script.src =
      "https://cdn.jsdelivr.net/npm/@novnc/novnc@1.5.0/dist/rfb.min.js";
    script.async = true;
  ...
  const vmAction = async (action: string) => {
    if (!name) return;
    await fetch(
      `${API_BASE}/api/v1/vms/${encodeURIComponent(name)}/${action}`,
      { method: "POST" },
    );
    ...
    <div className="h-full flex flex-col bg-black text-white">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-white/10 bg-black/80 backdrop-blur-sm">
        <div className="flex items-center gap-4">
          ...
          {vrde !== null && (
            <span
              className={`text-xs flex items-center gap-1 ${vrde ? "text-green-400" : "text-white/40"}`}
            >
              {vrde ? (
        ...
        <div className="flex items-center gap-2">
          ...
          {vrde && (
            <a
              href={`${API_BASE}/api/v1/vms/${encodeURIComponent(name!)}/rdp`}
        ...
        {!isRunning ? (
          <div className="flex flex-col items-center justify-center text-white/40">
            ...
            <p className="text-sm mt-2">
              Click <strong className="text-white/60">Start</strong> to boot.
            </p>
          </div>
        ) : !vncConnected ? (
          <div className="flex flex-col items-center justify-center text-white/40">
            ...
            <p className="text-sm mt-2 text-white/20">
              {!vrde
                ? "Enable Remote Desktop above"
                : "Establishing VNC session via noVNC"}
            </p>
          </div>
        ) : null}
        <div
          ref={containerRef}
          className={vncConnected ? "w-full h-full" : "hidden"}
        />
      </div>
    </div>
  );
