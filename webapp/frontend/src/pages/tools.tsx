      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Direct access to virtualization MCP capabilities
          </p>
        </div>
      </div>
              ...
              filteredTools.map((tool) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-2 rounded-lg ${selectedTool?.name === tool.name ? "bg-primary/20" : "bg-muted"}`}
                    >
                  ...
                  <ChevronRight
                    className={`w-4 h-4 transition-transform ${selectedTool?.name === tool.name ? "translate-x-1" : "opacity-0"}`}
                  />
                </button>
              ))
            )}
          ...
          {selectedTool ? (
            <div className="flex flex-col h-full">
              ...
              <div className="flex-1 p-6 overflow-auto space-y-6">
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          {selectedTool.inputSchema?.required?.includes(
                            ...
                            <span className="text-xs bg-primary/20 text-primary px-1.5 py-0.5 rounded uppercase">
                              required
                            </span>
                          )}
                        </div>
                ...
                {results && (
                  <div className="space-y-4">
                    <h4 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground text-primary">
                      Execution Result
                    </h4>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-12 text-center">
              ...
              <p className="mt-2 max-w-xs">
                Pick a virtualization primitive from the left to configure and
                execute its logic.
              </p>
            </div>
          )}
