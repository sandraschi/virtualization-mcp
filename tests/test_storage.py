...
from virtualization_mcp.services.vm.types import StorageBus, StorageControllerType, StorageMedium
...
class TestVMStorageMixin:
    ...
    def test_create_storage_controller(self, mock_vbox):
        ...
        result = self.storage.create_storage_controller(
            self.vm_name, "SATA Controller", StorageControllerType.PIIX4, StorageBus.SATA
        )
    ...
    def test_attach_storage_medium(self, mock_vbox):
        ...
        # Setup test data
        medium = StorageMedium(path="/path/to/disk.vdi", format_="vdi", type_="normal")

        # Call the method
        result = self.storage.attach_storage_medium(
            self.vm_name, "SATA Controller", port=0, device=0, medium=medium
        )
    ...
    async def test_create_disk_image(self, mock_vbox):
        ...
        result = await self.storage.create_disk_image(
            "/path/to/new/disk.vdi", size_gb=20, format_="vdi"
        )
    ...
    def test_list_attached_media(self, mock_vbox):
        ...
        # Call the method
        result = self.storage.list_attached_media(self.vm_name, "SATA Controller")
