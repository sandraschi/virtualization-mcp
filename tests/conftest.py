...
def mock_vbox_manager():
    ...
    manager = Mock()
    manager.list_vms = AsyncMock(return_value=[
        {"name": "TestVM1", "state": "running", "uuid": "uuid1"},
        {"name": "TestVM2", "state": "stopped", "uuid": "uuid2"}
    ])
    manager.get_vm_info = AsyncMock(return_value={
        "name": "TestVM",
        "state": "running",
        "memory": 4096,
        "cpus": 2,
...
def mock_network_manager():
    ...
    manager = Mock()
    manager.list_networks = AsyncMock(return_value=[
...
def mock_storage_manager():
    ...
    manager = Mock()
    manager.list_controllers = AsyncMock(return_value=[
...
def mock_system_manager():
    ...
    manager.get_vbox_version = AsyncMock(return_value={
        "version": "7.0.0",
        "build": "123456"
    })
    manager.list_ostypes = AsyncMock(return_value=[
...
def coverage_helpers():
    ...
    class CoverageHelpers:
        ...
        def assert_coverage_improvement(old_coverage: float, new_coverage: float, min_improvement: float = 0.1):
            ...
            assert improvement >= min_improvement, f"Coverage improvement {improvement:.2%} is less than required {min_improvement:.2%}"
